...
//##### Previous project kernels for correctness testing #####
#define n MAX_THRperBLK2D_MULTI
__global__ void phase1_multiy_test_krn(int* g, const int pstart, const int N){
...
__global__ void phase2_multiy_test_krn(int* g, const int pstart, const int primary_n, const int N){
  ...
  if(blockIdx.y)
    for(int k=0; k<n; k++){
      if(tile[2*threadIdx.y][threadIdx.x] >
          primary[2*threadIdx.y][k]+tile[k][threadIdx.x])
        tile[2*threadIdx.y][threadIdx.x]=
          primary[2*threadIdx.y][k]+tile[k][threadIdx.x];
      if(tile[2*threadIdx.y+1][threadIdx.x] >
          primary[2*threadIdx.y+1][k]+tile[k][threadIdx.x])
        tile[2*threadIdx.y+1][threadIdx.x]=
          primary[2*threadIdx.y+1][k]+tile[k][threadIdx.x];
      __syncthreads();
    }
  else
    for(int k=0; k<n; k++){
      if(tile[2*threadIdx.y][threadIdx.x] >
          tile[2*threadIdx.y][k]+primary[k][threadIdx.x])
        tile[2*threadIdx.y][threadIdx.x]=
          tile[2*threadIdx.y][k]+primary[k][threadIdx.x];
      if(tile[2*threadIdx.y+1][threadIdx.x] >
          tile[2*threadIdx.y+1][k]+primary[k][threadIdx.x])
        tile[2*threadIdx.y+1][threadIdx.x]=
          tile[2*threadIdx.y+1][k]+primary[k][threadIdx.x];
      __syncthreads();
    }
...
__global__ void phase3_multiy_test_krn(int* g, const int pstart, const int primary_n, const int N){
  ...
  for(int k=0; k<n; k++){
    ...
    if(tile[2*threadIdx.y+1][threadIdx.x] >
        col[2*threadIdx.y+1][k]+row[k][threadIdx.x])
      tile[2*threadIdx.y+1][threadIdx.x]=
        col[2*threadIdx.y+1][k]+row[k][threadIdx.x];
    __syncthreads();
  }
