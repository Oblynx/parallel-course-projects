...
Duration_fsec run_GPUsimple(MPIhandler mpi, const HPinPtr<int>& g, const int N, const unique_ptr<int[]>& groundTruth,
    FILE* logfile){
  DPtr<int> d_g(N*N);
...
#define MAX_THRpBLK2D 16
// GPU block algo
Duration_fsec run_GPUblock(MPIhandler mpi, const HPinPtr<int>& g, const int N, const unique_ptr<int[]>& groundTruth,
    FILE* logfile ){
  DPtr<int> d_g(N*N);
  cudaDeviceSynchronize();
...
Duration_fsec run_GPUblock_multixy(MPIhandler mpi, const HPinPtr<int>& g, const int N, const unique_ptr<int[]>& groundTruth,
    FILE* logfile){
  DPtr<int> d_g(N*N);
  cudaDeviceSynchronize();
  HPinPtr<int> result_block(N*N);
