...
void execPhase2Row (DPtr<int>& dsg, const int b,const int N, DPtr<int>& d_tile, int* rowbuf,
  ...
  if( executor == mpi.rank() ){
    ...
    xy rowStart= xy(0, n*b) - mpi.subStartXY();
    phase2Row( gs,bs, dsg, d_tile, rowStart, dsg.pitch_elt(), d_tile.pitch_elt() );
    dsg.copyD2H(rowbuf,mpi.s_x(), mpi.s_x(),n, rowStart);
    //dsg.copyD2H_multi(rowbuf, mpi.s_x(), n, rowStartXY);
    cuda.synchronize();

    PRINTF("[ph2row]: rowbuf: \trowStart=(%d,%d)\n", rowStart.x, rowStart.y);
...
void execPhase2Col (DPtr<int>& dsg, const int b, DPtr<int>& d_tile, int* colbuf,
  ...
  if( executor == mpi.rank() ){
    ...
    xy colStart= xy(n*b, 0) - mpi.subStartXY();
    phase2Col( gs,bs, dsg, d_tile, colStart, dsg.pitch_elt(), d_tile.pitch_elt() );
    ...
    PRINTF("[ph2col]: colbuf: \tcolStart=(%d,%d)\n", colStart.x,colStart.y);
